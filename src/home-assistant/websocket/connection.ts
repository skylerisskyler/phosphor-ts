...
import { HassEvent, MessageBase } from "./types";
...
export class Connection {
  ...
  setSocket(socket: HaWebSocket) {
    ...
    this.socket = socket;
    socket.on("message", (ev) => this._handleMessage(ev));
    socket.on("close", (ev) => this._handleClose(ev));
  ...
  private async _handleClose(ev: any) {
